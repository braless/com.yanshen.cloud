...
public class DateUtil {
     ...
     * @return
     */
/*    public static String getWeekOfYear(Date date,int weekStart) {
        WeekInfo weekInfo = getWeekInfo(date,weekStart);
        return weekInfo.getYearWeek();
    }*/
    ...
    /*   public static WeekInfo getWeekInfo(Date date, int weekStart) {
           WeekInfo weekInfo = new WeekInfo();
           weekInfo.setWeekStart(weekStart);
           *//*计算年周*//*
        Calendar calendar = getCalendar(date, weekStart);
        int weekOfYear = calendar.get(Calendar.WEEK_OF_YEAR);//获得当前日期属于今年的第几周
        int currentYear = calendar.get(Calendar.YEAR);
        *//*这里是防止出现周和年不对应的情况 比如2018年12月31日*//*
        calendar.add(Calendar.DAY_OF_MONTH, 6);
        int  weekYearByAfter7 = calendar.get(Calendar.YEAR);//6天之后的年份
        if(weekYearByAfter7 > currentYear && weekOfYear==1){
            currentYear+=1;
        }
        if(weekOfYear <10){
            weekInfo.setYearWeek(""+currentYear+"0"+weekOfYear);
        }else{
            weekInfo.setYearWeek(""+currentYear+weekOfYear);
        }
        //计算周开始时间和周结束时间
        Calendar cal = getCalendar(date, weekStart);
        int day = cal.get(Calendar.DAY_OF_WEEK)-1;// 获得当前日期是一个星期的第几天
        if(day ==0){
            day = 7;
        }
        if(weekStart<=day){
            cal.add(Calendar.DATE,weekStart-day);
        }else{
            cal.add(Calendar.DATE,weekStart-day-7);
        }
        weekInfo.setWeekStartTime(getDayStartTime(cal.getTime()));
        cal.add(Calendar.DAY_OF_YEAR, 6);
        weekInfo.setWeekEndTime(cal.getTime());
        return weekInfo;
    }*/
