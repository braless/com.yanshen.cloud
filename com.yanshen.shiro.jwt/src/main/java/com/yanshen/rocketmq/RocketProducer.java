...
import com.yanshen.util.CommonUtils;
...
import com.yanshen.util.MD5Util;
...
//        String signParam = vo.getCommand() + vo.getParam() + vo.getTimestamp() + md5key;
        String sign = "signtest";//MD5Util.stringToMD5(signParam);
        vo.setSign(sign);
    ...
    public void asyncSendMsg(String msgType, JSONObject data) {
        ...
        vo.setParam(CommonUtils.getAscString(data));
        String signParam = vo.getCommand() + vo.getParam() + vo.getTimestamp() + md5key;
        String sign = MD5Util.MD5Encode(signParam,"");
        vo.setSign(sign);
    ...
    public void asyncSendMsg(String msgType, String data) {
        RocketMsgVo vo = new RocketMsgVo();
        vo.setTopic(topic);
        vo.setTag(msgType);
        vo.setCommand(msgType);
        rocketMQTemplate.asyncSend(vo.getTopic() + ":" + vo.getTag(), JSONObject.toJSON(vo), new SendCallback() {
            @Override
            public void onSuccess(SendResult sendResult) {
                logger.warn("消息发送成功:{}", sendResult.getSendStatus());
            }

            @Override
            public void onException(Throwable throwable) {
                logger.warn("mqMsg={}消息发送失败,原因:{}", JSONObject.toJSONString(vo), throwable.toString());
            }
        });
    }
    ...
    public void sendMsgOrderly(String topic, String msgType, JSONObject data) {
        ...
        vo.setParam(CommonUtils.getAscString(data));
        String signParam = vo.getCommand() + vo.getParam() + vo.getTimestamp() + md5key;
        String sign = MD5Util.byteArrayToHexString(signParam.getBytes());
        vo.setSign(sign);
